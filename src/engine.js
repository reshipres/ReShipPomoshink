...
function routeDecision(classified, message, context) {
  ...
  switch (classified.intent) {
    ...
    case INTENTS.DELIVERY_TERMS:
      return answer('delivery_terms', 'Доставка доступна через CDEK: до пункта выдачи или курьером. Точный срок и стоимость считаются в корзине по городу и адресу. Отправка после оплаты обычно занимает 1-3 рабочих дня. Самовывоз в Москве доступен после подтверждения готовности заказа.', [
        'Где мой заказ?',
        'Адрес самовывоза',
        'Позови оператора',
      ], classified.confidence);
