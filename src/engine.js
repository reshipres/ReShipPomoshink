...
export function handleMessage({ message, session = {}, orderContext = null, productContext = null } = {}) {
  ...
  if (!text) {
    ...
    return {
      ...
      needsHandoff: false,
      handoffReason: 'none',
      suggestedReplies: ['Что есть в наличии?', 'Как доставляете?', 'Как оформить заказ?'],
      nextSession,
    };
  }
...
function routeDecision(classified, message, context) {
  ...
  switch (classified.intent) {
    case INTENTS.GREETING:
      return answer('greeting', 'Привет! Я помощник ReShip. Если вы впервые у нас, помогу выбрать товар в каталоге reship.pro, проверить наличие и цену, разобраться с доставкой, оплатой, самовывозом и оформлением. Если заказ уже есть, найду его по номеру, треку CDEK, телефону, email или ФИО получателя.', [
        'Что есть в наличии?',
        'Как доставляете?',
        'Как оформить заказ?',
    ...
    case INTENTS.ASSISTANT_IDENTITY:
      return answer('assistant_identity', 'Я помощник ReShip. Помогаю с выбором товара, наличием, ценой, доставкой, оплатой, самовывозом и оформлением. Для уже оформленных заказов могу проверить статус по номеру, треку CDEK, телефону, email или ФИО.', [
        'Что есть в наличии?',
        'Как доставляете?',
        'Где мой заказ?',
      ], classified.confidence);
    ...
    case INTENTS.BILLING_ISSUE:
      return handoff('payment', 'По оплате или возврату денег нужен оператор. Передаю вопрос, чтобы проверили платеж, возврат или статус заказа.', 'billing_issue', 'Проблема с оплатой или возвратом денег', `Клиент сообщает о проблеме оплаты/возврата: "${message}".`);

    case INTENTS.SITE_ISSUE:
      return handoff('site_issue', 'Похоже, проблема с сайтом, оформлением или уведомлениями. Передаю оператору, чтобы проверили вручную и помогли оформить заказ или разобраться с уведомлениями.', 'site_issue', 'Проблема сайта, оформления или уведомлений', `Клиент сообщает о проблеме сайта/оформления/уведомлений: "${message}".`);
    ...
    case INTENTS.REFUND_OR_RETURN:
      if (classified.detail === 'manufacturer_warranty') {
        return handoff('warranty_or_return', 'Гарантию производителя и отличие сроков лучше проверить вручную. Передаю оператору, чтобы уточнили по конкретному товару и ситуации.', 'manufacturer_warranty_review', 'Гарантия производителя', `Клиент спрашивает про гарантию производителя: "${message}".`);
      }

      if (classified.detail === 'return_condition_review') {
        return handoff('warranty_or_return', 'Возврат после проверки, распаковки или теста лучше разобрать вручную. Передаю оператору, чтобы не обещать лишнего по условиям.', 'refund_or_return', 'Условия возврата после проверки товара', `Клиент уточняет спорные условия возврата: "${message}".`);
      }
    ...
    case INTENTS.AVAILABILITY:
      ...
      return ask('availability', classified.hint
        ? classified.productDetail === 'restock_timing'
          ? 'Проверю поступление по этому товару. Если он есть в каталоге, посмотрю статус; если точного срока нет, передам оператору.'
          : 'Проверю наличие по этому товару. Если он есть в каталоге, подтяну остаток; если нет, передам оператору.'
        : classified.productDetail === 'restock_timing'
          ? 'Проверю поступление. Пришлите ссылку на товар, артикул или точное название модели; без этого срок безопасно не угадываю.'
    ...
    case INTENTS.PRODUCT_ADVICE:
      ...
      return ask('product_advice', 'Каталог ReShip: https://reship.pro. Помогу с выбором: напишите, что ищете — мышку, коврик, клавиатуру, глайды, свитчи или аксессуары. Если вопрос про speed/control, грипсы, совместимость или конкретную модель, пришлите название или ссылку — проверю наличие и цену, затем подскажу следующий шаг.', [
        'Проверить наличие',
        'Сколько доставка?',
        'Позови оператора',
    ...
    case INTENTS.GENERAL_HELP:
      return ask('general_help', 'Да, помогу. Напишите вопрос одним сообщением: это про товар, наличие, цену, доставку, оплату, самовывоз или уже оформленный заказ?', [
        'Что есть в наличии?',
        'Как доставляете?',
        'Где мой заказ?',
    ...
    case INTENTS.PAYMENT:
      return answer('payment', 'Оплата доступна картой МИР или через СБП на сайте. Наложенный платеж, оплата при получении, рассрочка, Долями и Сплит сейчас не входят в стандартные способы оплаты. Если деньги списались, а заказ не обновился, передам оператору.', ['Как оформить заказ?', 'Где мой заказ?', 'Позови оператора'], classified.confidence);
    ...
    case INTENTS.PICKUP:
      return answer('pickup', 'Самовывоз в Москве: Гончарный проезд, 8/40, м. Таганская. Выдача Пн-Вс с 14:00 до 16:00 после подтверждения готовности заказа.', ['Как оформить заказ?', 'Где мой заказ?', 'Позови оператора'], classified.confidence);
    ...
    case INTENTS.WARRANTY_OR_RETURN:
      return answer('warranty_or_return', 'Гарантия зависит от конкретного товара и производителя. Если нужен точный срок по модели, пришлите ссылку или название — проверю. Возврат товара надлежащего качества возможен в течение 7 дней при сохранении товарного вида и упаковки. Брак, повреждение, обмен или спорный случай передам оператору на проверку.', ['Позови оператора', 'Где мой заказ?'], classified.confidence);
