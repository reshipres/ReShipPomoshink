...
export function composeOrderInfoAnswer(detail = null) {
  if (detail === 'potential_delay') {
    return 'По общим правилам точные задержки нельзя угадывать без заказа или конкретного товара. Если вопрос про уже оформленный заказ, пришлите номер заказа, трек CDEK, телефон или полное ФИО получателя — проверю статус. Если вопрос про предзаказ или поставку, пришлите ссылку или точное название модели; если точного срока нет в базе, передам оператору.';
  }
