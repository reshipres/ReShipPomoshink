...
export const SUPPORT_FACTS = [
  ...
  },
  {
    ...
    keywords: ['оплата', 'сбп', 'карта', 'рассрочка', 'долями', 'сплит', 'наложенный', 'получении', 'деньги', 'средства', 'возврат денег', 'компенсация'],
    text: 'Стандартная оплата на сайте: карта МИР или СБП. Наложенный платеж, оплата при получении, рассрочка, Долями и Сплит сейчас не входят в стандартные способы оплаты. Если деньги списались, возврат не пришел, клиент спрашивает сколько вернут или просит компенсацию, нужен оператор.',
  },
  {
    id: 'warranty_policy',
    title: 'Гарантия и возврат',
    intents: ['warranty_or_return', 'refund_or_return', 'defect_or_damage'],
    keywords: ['гарантия', 'возврат', 'обмен', 'брак', 'сломано', 'дефект', 'вернуть', 'производитель', 'пленка', 'упаковка', 'микрик', 'кнопка', 'скрип', 'колесо'],
    text: 'Гарантия зависит от конкретного товара и производителя. Возврат товара надлежащего качества возможен в течение 7 дней при сохранении товарного вида и упаковки. Гарантию производителя, возврат после проверки/распаковки, брак, повреждение, обмен и спорные случаи нужно передавать оператору на ручную проверку.',
  },
  {
  ...
  },
  {
    ...
    text: 'Заказ можно искать по номеру заказа, CRM-номеру, треку CDEK, телефону, email или полному ФИО получателя. Одну фамилию можно использовать только как предварительный поиск: если есть риск однофамильцев, нужно запросить полное ФИО, точный номер или трек. Если длинный CRM-номер вида 4213R клиент пишет только цифрами, можно использовать числовую часть только при уникальном совпадении. Если найдено несколько заказов, нельзя угадывать: нужно запросить точный номер, трек, полное ФИО или уточнение.',
  },
  {
  ...
  },
  {
    ...
    keywords: ['оператор', 'менеджер', 'человек', 'изменить', 'отменить', 'ошибка', 'письмо', 'уведомление', 'брак', 'возврат', 'деньги', 'компенсация', 'снг', 'международная'],
    text: 'Оператор нужен для изменения заказа, отмены брони/заказа, спорной оплаты или возврата денег, ошибок сайта, проблем с письмами/уведомлениями, доставки по СНГ/за рубеж, внешних ссылок и ручного расчета, брака, возврата, обмена, претензий и случаев, где клиент сам просит человека.',
  },
];
