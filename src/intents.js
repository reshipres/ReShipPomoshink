...
export function classifyMessage(message, session = {}) {
  ...
  if (/(оператор|менеджер|жив(ой|ого)|человек|поддержк|позови|свяжите|жду.*ответ|жду.*информац|ожидаю.*ответ|нет.*ответа|вашего ответа|не отвечают|не ответили|ответьте|когда ответите|обратн.*связ|(?:сообщите|напишите|уведомите).{0,40}(?:в\s+)?(?:телеграм|telegram|тг)|(?:обещали|должн[аоы]?|жду|уже.{0,20}жду).{0,80}(?:ничего\s+)?не\s+пришл|не\s+пришли\s+еще.{0,80}(?:жду|месяц|недел|дн))/i.test(message)) {
...
function messageLooksLikeConfusion(message) {
  return /(последнее|выше|предыдущее|прошлое).{0,30}(не\s+понял|не\s+поняла|непонятно)|^(не\s+понял[а]?|не\s+понятно|непонятно)(?:\s|$)|не\s+совсем\s+понимаю|не\s+очень\s+понимаю|что\s+в\s+итоге\s+с\s+(?:этим|товаром|ним|ней)/i.test(message);
}
