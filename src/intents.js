...
function messageLooksLikeSiteIssue(message) {
  return /(сайт|корзин|оформлен|оформить|оформля|личн(ый|ом).*кабинет|аккаунт|промокод|кнопк).*(не работает|лежит|недоступ|ошибк|не могу|не получается|не дает|не даёт|не открывается|не отображ|не видно|трабл|проблем)|не могу.*(оформить|заказать|положить.*корзин|зайти.*(личн|кабинет|аккаунт)|войти.*(личн|кабинет|аккаунт))|ошибка.*(сайт|корзин|оформ|оплат|личн|кабинет|аккаунт)/i.test(message);
}
