...
function messageLooksLikeNewcomerEntry(message) {
  ...
  return /(я\s+(тут\s+)?перв(ый|ыи)\s+раз|впервые(?:\s+(у\s+вас|тут|здесь))?|новичок|новеньк|с\s+чего\s+начать|как\s+у\s+вас\s+все\s+работает|как\s+работает\s+(магазин|reship)|помогите\s+выбрать|что\s+можете\s+предложить)/i.test(text);
}

function messageLooksLikeSiteIssue(message) {
  return /(сайт|корзин|оформлен|оформить|оформля|ордер|личн(ый|ом).*кабинет|акк|аккаунт|промокод|кнопк).*(не работает|лежит|недоступ|ошибк|не могу|не получается|не дает|не даёт|не открывается|не отображ|не видно|трабл|проблем)|не могу.*(оформить|заказать|сделать\s+ордер|положить.*корзин|зайти.*(личн|кабинет|акк|аккаунт)|войти.*(личн|кабинет|акк|аккаунт))|(?:зайти|войти).{0,40}(личн|кабинет|акк|аккаунт).{0,40}не могу|ошибка.*(сайт|корзин|оформ|оплат|личн|кабинет|акк|аккаунт)|проблем[аы].{0,40}(с\s+)?(сайт|корзин|оформ|личн|кабинет|акк|аккаунт)/i.test(message);
}
