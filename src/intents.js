...
function messageLooksLikeAvailability(message) {
  ...
  return /(в наличии|в нале|на складе|есть ли|есть\?|есть.{0,30}(черн|бел|красн|син|розов|сер|фиолет|желт|зел|оранж|рыж)|(?:zero|зеро|mini|max|v\d+|модель|товар).{0,40}есть.{0,30}(черн|бел|красн|син|розов|сер|фиолет|желт|зел|оранж|рыж)|какие\s+(цвета|расцветки)|какой\s+цвет|осталось|остаток|когда будет|появится|появятся|появятся ли|появится ли|поступит|поступлен|поступлени|ожидается|ожидаются|ожидаете|поставка|завоз|дроп|предзаказ|под заказ|ресток|restock|доступен|можно заказать|будете завозить|привезете|привезёте|(?:не\s+)?будет\s+ли.{0,50}(в\s+)?продаже|будет.{0,50}(в\s+)?продаже|(?:нет\s+ли|есть\s+ли).{0,50}информац.{0,50}(дойти|прийти|поступ)|что\s+еще.{0,40}(должно|будет).{0,40}(дойти|прийти|поступить)|сколько.{0,40}(пришло|имеется|осталось|штук)|(?:он|она|они|его|ее|её|их|товар|модель).{0,40}(есть|нету|нет).{0,30}(в итоге|у вас|на складе)?|(?:есть|нету|нет).{0,30}(в итоге).{0,40}(он|она|они|его|ее|её|их|товар|модель)?)/i.test(message)
    || /(?:можешь|можно|сможешь|пожалуйста)?\s*(?:посмотреть|посмотри|проверить|проверь|глянь|погляди).{0,80}(?:мышь|мышк|ковр|клавиатур|глайд|свитч|товар|модель|superlight|beast|pulsar|vaxee|wlmouse|logitech|g\s*pro|xm2w)|(?:мышь|мышк|ковр|клавиатур|глайд|свитч|товар|модель|superlight|beast|pulsar|vaxee|wlmouse|logitech|g\s*pro|xm2w).{0,80}(?:посмотреть|посмотри|проверить|проверь|глянь|погляди)/i.test(message);
}
...
function messageLooksLikeDeviceDefect(message) {
  return /(брак|сломал|сломано|не работает|нерабоч|поврежд|разбит|дефект|микрофриз|прожимает|продавлив|двойн(ой|ые|ое).{0,40}(клик|нажат|срабатыв|регистр)|тройн(ой|ые|ое).{0,40}(клик|нажат|срабатыв|регистр)|сам[ао]?.{0,30}(нажим|клика|прожим)|(?:пкм|лкм).{0,80}(прожим|клика|нажим|проблем|скрип)|микрик.{0,80}(плох|функционир|скрип|нажим|клик|проблем)|кнопк[аи]?.{0,80}(продавл|плохо.{0,20}нажим|не\s+четк|не\s+ч[её]тк|скрип|проблем|фиксир|регистр|срабатыв)|клавиш[аи]?.{0,100}(фиксир|регистр|срабатыв|двойн|тройн|несколько)|нажат(ие|ия)?.{0,80}(фиксир|регистр|срабатыв).{0,80}(?:2|3|два|три|несколько)|(?:выдерг|отключ).{0,50}(провод|питани).{0,80}(нормаль|помога|проход)|колес(о|ик).{0,80}(не\s+прокруч|не\s+крут|скрип|проблем)|корпус.{0,120}(пластик|царап|скол|дефект|поврежд|дискомфорт)|(?:мыш[ьа]?|клавиатур[аы]?|клава).{0,120}(проблем|прожим|продавл|микрофриз|скрип|брак|дефект|фиксир|регистр|срабатыв)|не\s+хочу.{0,80}(играть|пользоваться).{0,80}(брак|дискомфорт|проблем)|плох(ая|ое|ой).{0,40}(четкость|ч[её]ткость).{0,40}нажат)/i.test(message);
}
