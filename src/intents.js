...
export function classifyMessage(message, session = {}) {
  ...
  if (/(оператор|менеджер|жив(ой|ого)|человек|поддержк|позови|свяжите|жду.*ответ|жду.*информац|ожидаю.*ответ|нет.*ответа|вашего ответа|не отвечают|не ответили|ответьте|когда ответите|обратн.*связ|(?:сообщите|напишите|уведомите).{0,40}(?:в\s+)?(?:телеграм|telegram|тг)|(?:обещали|должн[аоы]?|жду|уже.{0,20}жду).{0,80}(?:ничего\s+)?не\s+пришл|не\s+пришли\s+еще.{0,80}(?:жду|месяц|недел|дн|заканчивается|январ|феврал|март|апрел|ма[йя]|июн|июл|август|сентябр|октябр|ноябр|декабр)|слишком\s+долг(ое|о).{0,40}(ожидан|ждать)|долго.{0,40}(жду|ожидаю|не\s+пришл|нет\s+ответа))/i.test(message)) {
  ...
  if (/(не проходит оплат|не могу оплат|не получается оплат|ошибка оплат|оплатил.*статус|статус.*не измен|деньги списал|списали.*деньги|деньги\s+снял[ио]?|снял[ио]\s+деньги|деньги\s+ушли|двойн(ая|ое).*оплат|плат[её]ж.*не вижу|чек.*не приш|деньги.*(не.*вернул|не.*пришл|не.*компенс|возвращ)|когда.*деньги|деньги.*назад)/i.test(message)) {
    return match(INTENTS.BILLING_ISSUE, 0.98);
  }

  if (/(брак|сломал|сломано|не работает|нерабоч|поврежд|разбит|дефект|микрофриз|прожимает|двойн(ой|ые).{0,20}клик|сам[ао]?.{0,30}(нажим|клика|прожим)|(?:пкм|лкм).{0,80}(прожим|клика|нажим|проблем)|мыш[ьа]?.{0,80}(проблем|прожим|микрофриз))/i.test(message)) {
