...
console.log(anonymousMode
  ? 'ReShipPomoshink CLI. Анонимный клиент: ищу заказы только по введенным данным. Ctrl+C для выхода.'
  : 'ReShipPomoshink CLI. Известный клиент: могу сам найти последний заказ, также ищу по номеру, треку CDEK, телефону и фамилии. Ctrl+C для выхода.');

while (true) {
  const message = await rl.question('you> ');
  const result = handleCustomerMessage({ message, session, customer, orders });
